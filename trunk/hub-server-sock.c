		...
		if(IS_FREE(p))
		{
			...
			if(nodelay_flag)
			{
				if(0 != setsockopt(newsock, IPPROTO_TCP, TCP_NODELAY,
												&state, sizeof(state)))
					syslog(LOG_INFO, "[%ld/%d]Can't set TCP_NODELAY, errno is %d\n", 
											p-ccb, newsock, errno);
				else
			...
			syslog(LOG_INFO, "[%ld/%d]hs_sock_new_connect, %s\n", 
	...
	while(1==1)
	{
		...
		LOOP(in)							/* Visit all connects */
		{
			...
			if(FD_ISSET(in->socket, &recv_fd))
			{
				...
				if(n > tmp) 
					syslog(LOG_DEBUG, "[%ld/%d]Huh(0)? %d/%d\n", 
...
static void close_socket(connect_t *c, char *msg)
{
	if(c->socket < 0 || c->socket > (HS_MAX_CONNECT+3))
		syslog(LOG_DEBUG, "[%ld/%d]Huh(3)?\n", c-ccb, c->socket);
	
	if(msg && *msg)
		syslog(LOG_INFO, "[%ld/%d]close_socket: %s", c-ccb, c->socket, msg);
...
static void *out_threads(void *p)		/* 1 of these for each connect */
{
	...
	while(1)							/* Go forever */
	{
		...
		if(IS_CLOSING(out))
		{
			...
			syslog(LOG_INFO, "[%ld/%d]out_threads close: in: %.1e, outok: %.1e, outng: %.1e",
							out-ccb, out->socket, 
		...
		{	
			...
			if(j < 0)
			{
				syslog(LOG_DEBUG, "[%ld/%d]Huh(4)? %d/%d\n", 
			...
			if(j < i)
				syslog(LOG_INFO, "[%ld/%d]hsq_write_lines_from_queue: %d/%d\n",
							out-ccb, out->socket, j, i);
		}
	}
